...
const process = [
  ["01", "İlk temas", "Kısa bir mesajla ihtiyacınızı ve görüşme beklentinizi paylaşırsınız."],
  ["02", "İhtiyacı anlamak", "İlk görüşmede size iyi gelecek çalışma çerçevesini birlikte netleştiririz."],
  ["03", "Size uygun yol", "Sürecin nasıl ilerleyeceğine; ihtiyaç, hedef ve uygunluğa göre birlikte karar veririz."],
];
    ...
    <section className="shell py-20 sm:py-28">
      ...
      <div className="mt-14 space-y-4">
        {services.map(([title, text, items], index) => <article key={title as string} className="grid gap-6 rounded-3xl border border-[#17324f]/10 p-7 sm:p-10 lg:grid-cols-[.25fr_.75fr]">
          ...
          <div><h2 className="display text-3xl sm:text-4xl">{title}</h2><p className="mt-4 max-w-2xl leading-7 text-slate-600">{text}</p><ul className="mt-6 flex flex-wrap gap-2">{(items as string[]).map(item => <li key={item} className="rounded-full bg-[#efece6] px-3 py-1.5 text-sm text-[#17324f]">{item}</li>)}</ul></div>
        </article>)}
      </div>
    </section>
